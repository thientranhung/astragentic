...
|---|---|---|---|---|
| `AST-001` | Gate wired to a fix tool, not a review tool | promoted | 17 |   |
...
| `AST-028` | Relative worktree path + unverified pane cwd → worktree born in the wrong place, hour-long mi… | promoted | 189 | dispatch-ticket,review-with-rin |
...
| `AST-032` | A signal that cannot fail is not evidence | promoted | 770 | WATCHING.md,dispatch-ticket,dispatch-ticket-claude,dispatch-ticket-opencode,review-with-rin |
...
| `AST-045` | A green test suite and a coherent product are different claims | promoted | 331 |   |
| `AST-046` | A block moved between documents keeps the old document's referents | promoted | 143 |   |
| `AST-047` | "Local" is a deployment fact, not a data fact | promoted | 163 |   |
...
| `AST-050` | Qualify a plugin command always, not once it is known to collide | promoted | 217 | dispatch-ticket |
| `AST-051` | An address the caller cannot use produces a substitute, not an error | promoted | 319 | builder-claude.md,dispatch-ticket,thomas-claude.md |
| `AST-052` | The word-budget audit ran its loop zero times and reported all clean | promoted | 227 |   |
| `AST-053` | An axis read the run's verdict instead of its own, and went mute | promoted | 226 |   |
| `AST-054` | `git add -A` committed two releases nobody ever applied | promoted | 146 |   |
| `AST-055` | A gate that reads the subject cannot see which pass wrote it | promoted | 325 | builder-claude.md,builder-codex.md,builder-opencode.md,dispatch-ticket-claude,rin-claude.md,rin-codex.md,rin-opencode.md,rin.md,shaper.md,thomas-claude.md |
| `AST-056` | A blocking edge expresses order, not exclusion | promoted | 329 | dispatch-ticket,thomas.md |
| `AST-057` | A frontier that is only computed is invisible to the one person who cannot compute | promoted | 689 | reconcile-tracker,thomas.md |
...
| `AST-069` | An instruction with no moment attached measures zero | promoted | 272 | thomas.md |
...
| `AST-072` | Self-monitoring shipped without proof it cannot harm what it monitors | promoted | 345 |   |
...
| `AST-076` | AST-072's own fix left a window between acquiring the lock and recording who holds it | promoted | 1427 |   |
...
| `AST-082` | a copy-pasteable dispatch example taught the wrong pane name for one role | promoted | 182 |   |
| `AST-083` | the arm's own forbidden-character list was incomplete, and missed exactly the failure it exis… | promoted | 387 |   |
| `AST-084` | the watchdog trusted a pane title a runtime overwrites after launch | promoted | 326 |   |
| `AST-085` | a word budget calibrated against a retired file size passed nothing | promoted | 266 |   |
...
| `AST-092` | Builder stops after writing code but before committing — pane reads done, cleanup deletes the… | promoted | 308 | CLEANUP.md,builder.md |
| `AST-093` | A fix landing in .agents/skills/ but not .claude/skills/ is unreachable on the runtime that l… | promoted | 186 |   |
| `AST-094` | Builder commits and pushes correctly but silently skips the simplify pass | promoted | 426 | CLEANUP.md,builder.md |
...
| `AST-096` | rm -rf on a worktree directory leaves git's registration behind; the next add at that path re… | promoted | 296 | codex-arm,review-with-rin |
| `AST-097` | TERMINAL:done means the turn ended, not that the work finished | promoted | 831 | builder-claude.md,dispatch-ticket,dispatch-ticket-claude |
...
| `AST-099` | Simplify marker exists without skill provenance — the subject is self-applied, only the Pass:… | promoted | 685 | CLEANUP.md |
...
| `AST-107` | A long `herdr agent wait` stays alive and goes deaf, so the watch never fires | promoted | 328 | dispatch-ticket,dispatch-ticket-claude,thomas-claude.md |
...
| `AST-110` | A protocol change is an edit plus a sweep, and doc drift has at least three shapes | promoted | 417 |   |
...
| `AST-113` | An audit that always screams is an audit nobody reads | promoted | 290 |   |
| `AST-114` | Splitting submission into two steps left the watch armed against the wrong one | promoted | 207 |   |
| `AST-115` | Two correct changes composed into a live one, and the repair is what armed it | promoted | 447 | codex-arm |
| `AST-116` | A local fix that never goes upstream is a defect every fresh install re-buys | promoted | 335 |   |
| `AST-117` | A worktree isolates git, not a tool that writes to a fixed path | promoted | 329 |   |
| `AST-118` | A fallback that changes what the verdict MEANS, while keeping the same exit code | promoted | 373 |   |
...
| `AST-122` | Existence is not relationship: a verified pointer that proves nothing about what it points at | promoted | 481 | CLEANUP.md,builder.md,rin.md,thomas.md |
...
| `AST-126` | The check was scoped by a variable that means two different things | promoted | 326 |   |
