...
`bootstrap-glossary` runs once, by name, early in a brownfield project's life, alongside the other
invoked-once, owner-reviewed bootstrap passes: `extract-standards` for coding conventions and
`batch-triage` for an inherited backlog. Thomas owns all three as phases, so none of them becomes
work everyone assumes someone else ran. Once terms are `CONFIRMED`, `domain-modeling` is where
they get sharpened further. This skill's own job ends at handing `domain-modeling` a vocabulary
drawn from the code that is actually there, not one somebody guessed at in an interview.
