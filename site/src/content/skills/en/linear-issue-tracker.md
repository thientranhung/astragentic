...
oneLiner: "Drive Linear as the harness tracker, and check the tier's issue ceiling before building a pipeline."
group: adapter
order: 6
...
|---|---|
| The project's `issue-tracker.md` names Linear | `Skill(skill: "linear-issue-tracker")`, and no other adapter |
...
- Every invisible blocker, whether a deploy, a credential or a decision, was given an issue and
  an edge, because a blocker that is not an issue is invisible and the graph lies confidently.
...
- Nobody treated `list_agent_skills` as writable, because that shelf is authored by the owner in
  Linear's own interface.
