...
oneLiner: "Drive GitHub Issues as the harness tracker, where status is a label and the board is a mirror nothing polishes."
group: adapter
order: 4
...
`github-issue-tracker` is one of three tracker adapters. `.agents/tracker-contract.md` states
what the pipeline requires of *any* tracker — five things, no more — and this file is the HOW for
one of them: status as labels, the claim, native dependencies, the frontier loop and the Projects
board mirror. The project half stays in the project: the `<owner>/<repo>`, the ticket prefix and
the label-to-column names live in that project's `docs/agents/issue-tracker.md`. It is reached
with the `gh` CLI rather than an MCP server, because `gh` is already authenticated wherever pull
requests are already being driven.
<!-- source: harness/.agents/skills/github-issue-tracker/SKILL.md -->

One fact shapes everything else: **GitHub Issues has no status field.** Status lives in two
unsynced places — the label, which is the truth, and the Project `Status` column, which is a
mirror somebody has to write. Nothing in GitHub syncs one to the other, so every status change
is two writes, forever. That is the standing cost of the cheapest tracker to start on. Every trap
on this page was paid for on a live project that moved from Linear to GitHub on 2026-08-21.
...
|---|---|
| The project's `issue-tracker.md` names GitHub | `Skill(skill: "github-issue-tracker")`, and only that one |
...
- **`gh` carries the `project` OAuth scope**, which is not in its default set. Without it,
  `--json projectItems` returns `[]` rather than an error, which is indistinguishable from "this
  issue is on no board". The owner runs `gh auth refresh -s project` once.
- **The project's `issue-tracker.md` exists** and carries the repo, the ticket prefix, the
  label-to-column names, which tracker it came from, and the pull-request-as-request-surface
  decision.
...
- **AST-057**: a frontier that is only computed is invisible to the one person who cannot
  compute. An agent re-runs the query on demand and never notices anything missing; the owner
  opens the board and looks. Measured on a live project: zero issues had ever entered the
  unstarted state across the project's whole life. Fixed in the contract as two halves — write
  the computed answer back as state, and never read a readiness label as a blocker — plus a merge
  step that must be reported, where `none` is a valid report and silence is not.
- **AST-074**: a tracker measured only against itself cannot detect its own drift. Four tickets
  ...
  full day, and nothing errored. A wrong state is perfectly consistent with itself, so the oracle
  has to be independent of what it measures. Fixed: `reconcile-tracker` plus
  `scripts/ticket-git-facts.sh`, read-only by a recorded ruling.
...
and writes `todo` on whatever it unblocked, in the same breath → `reconcile-tracker` measures the
result against git. Its siblings are `jira-issue-tracker` and `linear-issue-tracker`, and
