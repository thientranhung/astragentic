---
title: codex-arm
oneLiner: "Fire a Codex pass over a finished artifact so a different vendor reads the diff before it merges."
group: gate
order: 1
...
`codex-arm` is the invocation mechanics for the cross-vendor arm: the Codex review Thomas, or
at ticket scope the Builder itself, fires over a completed artifact before it can merge. It
...
It never decides *when* to fire. That cadence, and the two-pass cap, belong to `thomas.md` and
`builder.md`. This skill owns the how, not the when.
<!-- source: harness/.agents/skills/codex-arm/SKILL.md -->

The friction it answers is that a same-vendor reviewer and a cross-vendor one catch different
defect classes, because the author reads the ticket and the arm reads the repository. Measured
directly: the first per-ticket arm run caught a destructive reset authorizing outside its write
...
|---|---|
| A ticket's diff is committed, root is Claude or the Builder is firing its own ticket arm | `codex-arm` |
| Root runtime is Codex, a Claude pass is needed instead | `codex-claude-arm` (mirror skill; arm always calls the *other* vendor) |
| A spec is finished, about to be cut into tickets | `codex-arm` at `arm: spec`, fired by Thomas from the base checkout |
| A slice is closing | `codex-arm` at `arm: slice`, same as above |
| Rin's own gate needs to run | Not this skill. Rin never fires the arm from inside its own review |
...
- At ticket scope, the Builder runs it from its own worktree, where `HEAD` already resolves to
  the reviewed commits. At spec or slice scope, Thomas resolves the head itself from a detached
  checkout at that SHA, because `--base` alone doesn't say which head the companion compares
  against. <!-- source: harness/.agents/skills/codex-arm/SKILL.md -->
- Focus text avoids every shell/glob-special character, not a fixed list. It's passed unquoted,
  word by word, so a naturally-written phrase like `option (a)` never reaches the process; zsh's
  glob expansion kills the command at parse time. <!-- source: harness/.agents/skills/codex-arm/SKILL.md -->
...
| The arm range | First line of output: commit and file count, so a 0-commit range can't pass as clean |
...
- **AST-103**: the companion resolves `HEAD` from the checkout it runs in; where that disagrees
  ...
  the operator, not the gate. Fixed: print the commit/file range as the first output line, stop
  on 0 commits.
- **AST-095**: the companion exits 0 on configuration failure and caches state keyed to the
  workspace root. Fixed: never branch on exit code, only the output file's content; never reuse
  a gate worktree path.
...
- The recorded vendor in the merge trail matches the vendor that actually executed, never a
  same-vendor lens counted as the cross-vendor pass.
- No gate worktree survives past its review. Resources released before removal, on every path.
- A blocking finding from pass 1 gets a pass 2 over the fix, not a judgement call to skip it.
...
before removing the worktree. At spec and slice scope, Thomas fires it from the base checkout
before releasing a paused `shaper` or closing a slice. `codex-claude-arm` mirrors it for a Codex
root, same cadence, opposite vendor, and neither substitutes for `/skills/review-with-rin`,
which is the gate itself rather than the pass feeding it.
