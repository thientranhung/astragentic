...
oneLiner: "Launch an OpenCode pane and say out loud which of the shared protocol's checks degrade here."
group: adapter
order: 3
runtimes: [opencode]
...
runtime facts. Only two of the six are conveniences. The rest are limits, and the file states
them as limits rather than working around them.
<!-- source: harness/.agents/skills/dispatch-ticket-opencode/SKILL.md -->

The fact that shapes the page is that **OpenCode's `idle` is fabricated.** `herdr agent explain`
reports `fallback_reason: default_known_agent_idle_fallback`, and `agent wait --until idle`
returned rc=0 in 8 ms on a pane nobody had touched. Its manifest carries three rules against
Claude's twelve and Codex's seven, covering only `blocked` and `working` — so idle is not
detected, it is what is left when no rule matches. Two consequences follow and the second is the
expensive one: the start guard still works, terminal-state detection does not; and because a
transcript read here returns only the input box and the footer, the shared protocol's two-source
background check silently collapses to `pgrep` alone. This adapter's job is to make that
collapse visible instead of letting it pass as a normal answer.
<!-- source: harness/.agents/skills/dispatch-ticket-opencode/SKILL.md -->
...
| An OpenCode row with a non-blank Effort cell | Stop and ask the owner — effort is unreachable here |
| A pane that reports `idle` | Read the artifact; the state carries no rule behind it |
| A worktree about to be removed on a single-source answer | Get the second source another way, or hand back instead |
...
- **The role adapter exists in the worktree**: `test -f <worktree-path>/.opencode/agents/<role>.md`.
  ...
  `opencode agent list --pure`, which does not show project agents.
...
- **The Effort cell for this row is blank**, because effort and orchestration visibility are
  mutually exclusive on OpenCode and visibility wins.
...
- No verdict rests on `idle` alone; every finish claim points at an artifact.
...
runtimes → the artifact, not the pane, decides whether the ticket is done. Its siblings are
`dispatch-ticket-claude` and `dispatch-ticket-codex`.
