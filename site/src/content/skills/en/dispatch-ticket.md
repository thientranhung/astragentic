...
`dispatch-ticket` is the sequence Thomas runs every time a ticket goes from "claimable" to
"a Builder is actually working on it": claim it on the tracker, cut a branch and worktree, open
a Herdr tab and pane, print the resolved dispatch, hand over the brief, and arm a watcher before
moving on. Nine steps, always in that order. A step nobody checks is a step that gets skipped
silently, and this harness runs several of these in parallel, so silently here means a Builder
idle in a pane nobody is watching. <!-- source: harness/.agents/skills/dispatch-ticket/SKILL.md -->

What's different about it is where the boundary lives: **the claim happens before the worktree
exists.** A ticket gets assigned on the tracker first, so two Thomas sessions picking off the
same frontier see each other's claims instead of racing to create the same branch. The friction
this solves showed up as two sessions sharing one checkout and silently losing commits to a
concurrent `git switch`. One worktree per session is the fix, and isolation covers every
command that writes to disk, not only git. <!-- source: harness/.agents/skills/dispatch-ticket/SKILL.md -->
...
| A Shaper, QA or Rin needs a pane, not just a Builder | Same skill, same mechanics; only the tab/pane label prefix changes |
| A pane went idle or blocked mid-turn | `WATCHING.md` (companion reference, not a separate skill) |
| The Builder handed back and it's time to remove the worktree | `CLEANUP.md` (companion reference) |
...
- The workspace watchdog is running for this project. A dispatch with no watchdog is a hard
  stop, not a warning (`exit 1`, not an `echo`). <!-- source: harness/.agents/skills/dispatch-ticket/SKILL.md -->
...
- `.agents/orchestrator.md` has a real row (runtime, model, effort) for the role being
  ...
  <!-- source: harness/.agents/skills/dispatch-ticket/SKILL.md -->
- Role contract for the dispatched role read first (`builder.md` for a Builder), since the
  skill only carries the mechanics, not what the role is supposed to do with them.
...
| The brief | Sent as the first line a slash command, plugin-qualified (`/mattpocock-skills:implement <ticket>`) |
...
- **AST-036**: a harness allow-listed but not committed is invisible inside every Builder
  worktree. Fixed: commit check before first dispatch.
- **AST-032 / AST-037**: a multi-line brief pastes into the composer without submitting, and the
  pane reports `idle` while it sits unsent. Fixed: explicit Enter after paste, plus requiring the
  watcher to observe `working` before believing the turn began.
...
- **AST-092**: a Builder that stops before committing leaves work that only exists on disk;
  `git worktree remove` deletes it silently. Fixed: "commit, push, then return" as three explicit
  actions.
...
- Every submitted brief has a watcher armed against it within the same action, not a separate
  step done "after."
- A pane's tab and pane labels match the role dispatched (`builder:<id>` vs `spec:<id>` vs
  `qa:<id>` vs `rin:<id>`), a wrong prefix is invisible to the watchdog.
