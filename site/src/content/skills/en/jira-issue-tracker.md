...
oneLiner: "Drive Jira as the harness tracker, where a status is a numbered transition you read before every write."
group: adapter
order: 5
...
in that project's `docs/agents/issue-tracker.md`. I reach Jira through the Atlassian MCP tools.
Everything here was measured on a live team-managed project that moved from Linear to Jira on
2026-08-21.
<!-- source: harness/.agents/skills/jira-issue-tracker/SKILL.md -->
...
|---|---|
| The project's `issue-tracker.md` names Jira | `Skill(skill: "jira-issue-tracker")`, and no other adapter |
| A status change | `getTransitionsForJiraIssue`, then `transitionJiraIssue` with an id from *that* response |
| A description to patch | Read it, substitute, send the whole field back, because there is no partial update |
...
- **The `cloudId` is written down.** It appears in no URL a human pastes, so without it every
  session starts with a lookup through `getAccessibleAtlassianResources`.
...
- **A tracker measuring only itself missed drift.** A tracker measured only against itself cannot detect its own drift. Four tickets
  ...
  ruling, because the join key is a ticket id in a commit subject and a citation is not a
  completion.
...
- `Relates` links were kept out, because a mention in the body carries the same information where
  it is readable.
...
git. The two adapters in the same group are `github-issue-tracker` and `linear-issue-tracker`,
and `.agents/tracker-contract.md` sits above all three.
