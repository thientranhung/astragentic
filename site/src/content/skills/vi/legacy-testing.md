...
oneLiner: "Ghim lại thứ code chưa có test đang làm hôm nay, cắt một seam vào nó, rồi mới cho TDD bắt đầu."
group: brownfield
order: 1
...
trước, và điều đó giả định có sẵn một **seam** — một chỗ để anh em thay thế thứ mà code phụ thuộc
vào. Code đang tồn tại thường không có chỗ nào như vậy: hàm với thẳng tới đồng hồ, tới mạng, tới
database, hay tới một singleton ở cấp module. Skill này chốt thứ tự cho ca đó, và thứ tự ấy ngược
với greenfield: characterise xem code đang làm gì, tạo một seam, rồi mới chạy `tdd` bình thường
với các characterisation test làm lưới đỡ bên dưới.
<!-- source: harness/.agents/skills/legacy-testing/SKILL.md -->

Rắc rối nó gỡ là một Builder đứng khựng trên một ticket mà không có gì để viết test đối chiếu, và
hai lối thoát tệ ra khỏi cú khựng đó. Lối thứ nhất là một characterisation test âm thầm khẳng
định thứ code *đáng lẽ* phải làm: nó đỏ ngay ngày đầu và không nói gì cho anh em về thứ nào an
toàn để đổi. Lối thứ hai tệ hơn và im hơn — một test lặng lẽ phong một con bug thành chủ ý, và đó
chính là cách một con bug trở thành một yêu cầu. Nên kỷ luật ở đây nằm trong một cái comment: cứ
khẳng định đúng cái giá trị gây ngạc nhiên đó, và đánh dấu nó là đã-ghim-nhưng-chưa-phán kèm một
mã ticket, để cả hai cách đọc cùng còn sống. Skill này cũng là lý do brownfield ở đây được coi là
mặc định chứ không phải ca đặc biệt; các agent skill ở thượng nguồn đều giả định seam có sẵn, còn
phần lớn repo đi tới thì không có.
<!-- source: harness/.agents/skills/legacy-testing/SKILL.md, README.md -->

## Khi nào Thomas gọi nó

Thomas hiếm khi gọi thẳng cái này. Nó là **craft do model tự gọi**, chạm tới khi tình huống xuất
hiện, và lúc nhận repo thì không cần đấu dây gì ngoài việc xác nhận nó đã được đặt sẵn.
...
| Bán kính ảnh hưởng cứ phình ra trong lúc anh em đọc | Không phải cái này. `untangle` mới là đường khi bản thân mớ rối là vấn đề |
| Một refactor ở nơi module boundary đã có sẵn | `mattpocock-skills:improve-codebase-architecture`, cái đó có sẵn thứ để làm việc cùng |
...
- Dữ liệu coverage, trước khi chọn input. Input cho characterisation được chọn **theo coverage,
  ...
  thì hiếm khi đáng; characterise những đường mà thay đổi có thể làm hỏng thì luôn đáng.
- Một mã ticket để tham chiếu từ các comment đã ghim, vì một con bug bị ghim là một ticket về sau
  chứ không phải một cú xoá.
- Ý thức rằng một seam lớn không phải chỗ anh em tự quyết. Thêm một tham số vào một hàm là việc của
  Builder. Một interface mới mà vài module sẽ phụ thuộc vào thì đang định hình module boundary, và
  chuyện đó thuộc về nơi có cả bức tranh trong ngữ cảnh.
...
| Một giá trị gây ngạc nhiên mà anh em vẫn khẳng định | Một comment `// CHARACTERISATION:` nói rõ đã ghim gì, rằng nó chưa được phán là đúng, và mã ticket |
...
| Một seam quá lớn cho một ticket | Một cú handback cho Thomas, gọi tên các đường code, chỗ tắc, và cái seam nhỏ nhất anh em nhìn thấy |
...
- Việc tạo seam và việc đổi hành vi không bao giờ chung một commit. Nếu test vỡ, anh em cần biết
  chắc đó là do cái seam.
