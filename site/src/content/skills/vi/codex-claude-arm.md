...
oneLiner: "Chạy một lượt Claude đọc lại artifact đã xong, khi phiên gốc chạy trên Codex."
group: gate
order: 3
runtimes: [codex]
...
`codex-claude-arm` làm đúng một việc: lượt `claude -p` mà một phiên gốc Codex chạy trên một
...
lại*, nên trên phiên gốc Codex nó gọi Claude.
<!-- source: harness/.agents/skills/codex-claude-arm/SKILL.md -->
...
Hãy mang nó lên hỏi chủ dự án thay vì tìm cách lách. Thứ một phiên gốc Codex đóng góp được là lượt
...
| Builder trên phiên gốc Codex vừa xong một ticket | Chính Builder chạy `arm: ticket` |
| Một spec đang tạm dừng, hoặc một slice vừa đóng, trên phiên gốc Codex | Thomas chạy `arm: spec` / `arm: slice` |
...
- **Artifact đã xong và đã bàn giao.** Chạy sớm là tiêu một trong hai lượt được phép vào thứ
  còn đang thay đổi.
...
Artifact đã commit và đã bàn giao → `codex-claude-arm` chạy một lượt Claude qua đúng khoảng đã
