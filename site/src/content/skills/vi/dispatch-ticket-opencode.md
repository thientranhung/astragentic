...
oneLiner: "Khởi chạy một pane OpenCode và nói thẳng những bước kiểm nào của giao thức chung bị yếu đi ở đây."
group: adapter
order: 3
runtimes: [opencode]
...
trong sáu là tiện ích. Bốn cái còn lại là giới hạn, và file này ghi chúng như giới hạn thay vì
tìm cách đi vòng.
<!-- source: harness/.agents/skills/dispatch-ticket-opencode/SKILL.md -->

Sự thật định hình cả trang này là **`idle` của OpenCode là bịa.** `herdr agent explain` báo
`fallback_reason: default_known_agent_idle_fallback`, và `agent wait --until idle` trả về rc=0
sau 8 ms trên một pane không ai đụng vào. Manifest của nó có ba luật, so với mười hai của Claude
và bảy của Codex, và chỉ phủ `blocked` với `working` — nên idle không được phát hiện, nó là phần
còn lại khi không luật nào khớp. Kéo theo hai hệ quả, và cái thứ hai mới đắt: bộ chặn khởi động
vẫn chạy được, còn phát hiện trạng thái kết thúc thì không; và vì đọc transcript ở đây chỉ trả
về ô nhập liệu với dòng chân trang, bước kiểm nền hai-nguồn của giao thức chung lặng lẽ co lại
còn mỗi `pgrep`. Việc của adapter này là làm cú co đó lộ ra, thay vì để nó đi qua như một câu
trả lời bình thường.
<!-- source: harness/.agents/skills/dispatch-ticket-opencode/SKILL.md -->
...
| Một dòng OpenCode có ô Effort không trống | Dừng và hỏi chủ dự án — effort không với tới được ở đây |
| Một pane báo `idle` | Đọc artifact; trạng thái đó không có luật nào đứng sau |
| Một worktree sắp bị gỡ dựa trên câu trả lời một-nguồn | Kiếm nguồn thứ hai bằng cách khác, hoặc bàn giao thay vì gỡ |
...
- **Adapter của vai có mặt trong worktree**: `test -f <worktree-path>/.opencode/agents/<role>.md`.
  ...
  bằng `opencode agent list --pure`, vì lệnh đó không hiện agent cấp dự án.
...
- **Ô Effort của dòng này để trống**, vì trên OpenCode effort và khả năng nhìn thấy pane loại
  trừ nhau, và khả năng nhìn thấy thắng.
...
| Một bước kiểm kết thúc bị yếu đi | Một báo cáo suy giảm, viết giống như báo runtime rơi về dự phòng, không phải một phán quyết một-nguồn im lặng |
...
- **AST-097**: `TERMINAL:done` nghĩa là lượt đó đã kết thúc, không phải công việc đã xong, và
  ...
  nguồn transcript lẽ ra để đối chứng lại không trả về gì trên OpenCode.
...
- Không phán quyết nào dựa vào mỗi `idle`; mọi lời khẳng định đã xong đều chỉ vào một artifact.
...
quyết của nó yếu hơn hai runtime kia → artifact, chứ không phải pane, mới quyết định ticket đã
xong hay chưa. Hai skill anh em của nó là `dispatch-ticket-claude` và `dispatch-ticket-codex`.
