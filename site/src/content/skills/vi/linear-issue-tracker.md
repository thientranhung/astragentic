...
oneLiner: "Chạy Linear làm tracker của harness, và kiểm trần issue của gói đang dùng trước khi dựng pipeline lên nó."
group: adapter
order: 6
...
pipeline đòi ở bất kỳ tracker nào; file này là phần CÁCH LÀM cho Linear. Đây là adapter mỏng
...
đều được đáp ứng, nên phần lớn thứ mà hai adapter kia phải dài dòng ở đây không tồn tại. Chạm vào
nó qua bộ công cụ MCP `linear-server`, đừng gọi CLI. Đo trên một dự án thật chạy Linear tới
2026-08-21.
<!-- source: harness/.agents/skills/linear-issue-tracker/SKILL.md -->

Có hai thứ nên biết trước khi chọn nó, và thứ nhất chẳng liên quan gì tới mô hình dữ liệu. **Gói
free ngừng nhận issue mới.** `save_issue` trả về *"You've exceeded the free issue limit"*, và cái
đó không làm pipeline yếu đi — nó làm pipeline dừng, vì bước đầu tiên của phương pháp là `create`.
Một dự án đâm vào đúng chỗ này và phải di trú ngay trong ngày. Yêu cầu 1 tới 5 nói về *mô hình*
của một tracker; một tracker còn phải chịu nhận lệnh ghi nữa, nên kiểm trần của gói đang dùng
trước đã. Thứ hai là giao thức claim, ở đây yếu hơn mức phương pháp giả định, và git mới là thứ
gánh nó.
<!-- source: harness/.agents/skills/linear-issue-tracker/SKILL.md -->
...
|---|---|
| `issue-tracker.md` của dự án ghi Linear | `Skill(skill: "linear-issue-tracker")`, và chỉ mình nó |
| Tạo, đọc, gắn label, nối quan hệ hoặc đóng | `save_issue`, gần như là toàn bộ bề mặt |
| Một tài liệu bản đồ cho một đợt việc | `save_document` trên Linear project — một nguyên thuỷ hạng nhất, không phải cách lách của GitHub |
| Một ticket đọc ra là sẵn sàng trong khi epic cha của nó đang bị chặn | Lọc thêm theo trạng thái cha; chặn không di truyền xuống con |
| Tìm trong chính issue của mình | `list_issues` kèm `query`; `search_documentation` là tài liệu sản phẩm của Linear |
...
- **AST-057**: một frontier chỉ được tính ra thì vô hình với đúng người không tính được nó. Đo
  ...
  biểu diễn độ sẵn sàng nằm cạnh nhau mà không cái nào trả lời được câu hỏi của người dispatch.
  Đã sửa trong hợp đồng: ghi kết quả tính được trở lại thành trạng thái, và không bao giờ đọc một
  label sẵn-sàng như một cái chặn.
- **AST-074**: đẩy ticket lên frontier chỉ dựa vào cạnh chặn thì bao quá rộng, đo lại được trên
  Linear khi ba ticket nổi lên như claim được ngay giữa một phase trước đó — một sub-issue không
  có cái chặn nào đọc ra là sẵn sàng ngay cả khi epic cha của nó đang bị chặn. Đã sửa: việc đẩy
  lên frontier vẫn là phán đoán của người điều phối, ghi thẳng trong `thomas.md`, không phải kết
...
- Mọi cái chặn vô hình — một lần deploy, một credential, một quyết định — đều được cấp một issue
  và một cạnh, vì một cái chặn không phải issue thì vô hình và đồ thị sẽ nói dối rất tự tin.
...
- Không ai coi `list_agent_skills` là ghi được; kệ skill đó do chủ workspace soạn trên chính giao
  diện Linear.
...
anh em là `github-issue-tracker` và `jira-issue-tracker`, và `.agents/tracker-contract.md` đứng
trên cả ba.
