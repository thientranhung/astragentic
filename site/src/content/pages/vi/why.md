...
Cả hai có chung một điểm nghẽn. Câu hỏi kỹ thuật của agent thường vượt quá chuyên môn của
...
Cả hai cơ chế xuất phát từ một nguyên tắc thiết kế: Astragentic là scaffold, không phải quy trình
...
được. Tracker là chỗ duy nhất thoả cả hai vế, nên ADR-0001 gọi nó là nền điều phối của
...
được tính đúng nhưng chỉ tồn tại trong context của agent. Vì vậy contract mang cả hai nửa: tính xong thì ghi
...
status được lưu trong label và cột trên Project board chỉ là bản sao phải giữ đồng bộ. Mỗi adapter vì
...
không đủ để điều phối một team, vì bốn điều còn thiếu có chung một đặc điểm: khi có lỗi, không có tín
hiệu nào báo ra.
...
Không có tracker giữ trạng thái. Trạng thái của subagent nằm trong context của session cha, mất
đi khi session compact, và trong lúc tồn tại thì bạn không đọc được.
...
cuối: quyết định chưa chốt được đưa vào code, rồi mới chốt ở khâu review, là khâu tốn kém nhất.
...
lại. Nguyên nhân không nằm ở reviewer mà ở việc quyết định được chốt quá muộn.
...
một repo là hai bộ điều phối cùng quản lý một trạng thái, nên Astragentic không kết hợp.
...
nằm ở `to-tickets`, và cách xử lý đúng là để contract tính tới defect đó thay vì fork một bản
vá.
...
đã khai, điều người viết code khó nhận ra vì đang làm việc theo yêu cầu của ticket.

Số đo: AST-015, một vòng review cùng vendor cho lọt một defect đem secret đang dùng thật và PII của người
...
treo im lặng, nên cần timeout và một dispatcher theo dõi. Nguy hiểm nhất là phạm vi: nếu
