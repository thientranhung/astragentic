...
owns. The cost is that you run one more agent session and read what it intends to do, instead
of typing one command and walking away.

## brownfield

These four skills exist because real repos are rarely clean. `bootstrap-glossary` pulls domain
vocabulary out of the code itself; each term carries the file it was read from and stays marked
unreviewed until the owner confirms it. The rule is extract, never invent: a glossary an agent
imagined but wrote in a confident voice is more dangerous than no glossary at all.

`batch-triage` takes an inherited backlog in one pass instead of one ticket at a time.
`legacy-testing` builds a seam into code that offers nothing to test against. `untangle` is for
repos with no module boundaries left to improve, where a clean restructure would arrive as one
