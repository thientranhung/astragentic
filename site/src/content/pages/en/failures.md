...
**Cost.** A busier router that sometimes over-claims. I take that trade, because an idle slot
...
**Cost.** One extra pass every time a ticket closes.
...
**Cost.** Orphaned worktrees piling up and the occasional manual sweep. Next to losing a day of
...
**Cost.** A value that has touched a tracked file is burned and has to be rotated. Downstream
...
**Cost.** A reconcile step nobody enjoys running, and most of the time it finds nothing.
...
**Cost.** Fewer tickets running at once, which is the exact thing this whole system exists to
increase.
