...
- Fire `arm: ticket` from the Builder's own worktree. This is the one gate whose trigger sits with
  this role.
...
- Never commit without checking `git branch --show-current`; a branch switch can happen between two
  turns.
...
- Never use `Unreviewed-delta:` for code from a phase that had not run yet; that part owes a fresh
  gate.
